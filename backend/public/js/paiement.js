...
var SyntaAPI = isInIframe && window.parent.SyntaAPI ? window.parent.SyntaAPI : window.SyntaAPI;
...
/**
 * Get the current user from the server-injected profile.
 * The app uses HttpOnly cookie auth — Supabase JS client has no session.
 * The server injects window.__SYNTA_PROFILE__ on every protected page.
 */
function getServerUser() {
    const profile = window.__SYNTA_PROFILE__ || window.appUserState;
...
async function loadTransactions() {
    ...
    try {
        ...
        // Fetch all data from the backend API (uses cookie auth — no Supabase JS session needed)
        const response = await fetch('/api/user/transactions', { credentials: 'same-origin' });
        if (!response.ok) {
            throw new Error(`Server returned ${response.status}`);
        }
        const { transactions, enrollments, payments } = await response.json();
